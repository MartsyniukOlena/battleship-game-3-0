...
class Board:
    ...
    Has methods for adding ships and guess and printing the board
    """
    ...
    def print(self):
        """
        Print the board with ships and guess.
        """
        print("  " + "  ".join(str(i) for i in range(1, self.size + 1)))
        for i, row in enumerate(self.board, start=1):
            print(i, "  ".join(row))

    def add_guess(self, x, y):
        """
        Add player's guess to the board.
        """
...
def play_game(comp_board, player_board):
    """
    ...
    while True:
        ...
        comp_board.print()
        make_guess(comp_board)  # Player makes a guess on computer's board

        # Check if all ship coordinates are guessed
        # by the player on the computer's board
        if all(coord in comp_board.guess for coord in comp_board.ships):
        ...
        # and avoids repeating guess
        while True:
            x, y = random_point(comp_board.size), random_point(comp_board.size)
            if (x, y) not in computer_guess:
                computer_guess.add((x, y))
                break

        # Computer makes a guess on player's board
        result = player_board.add_guess(x, y)
        if result == "Hit":
        ...
        if all(coord in player_board.guess for coord in player_board.ships):
            print(f"Your score: {scores['player']}")
            print(f"Computer's score: {scores['computer']}")
...
def new_game():
    ...
    play_game(comp_board, player_board)
